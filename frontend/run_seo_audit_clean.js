...
(async () => {
  ...
  for (const item of redirectUrls) {
    console.error(`Testing redirect: ${item.url}`);
  ...
  for (const url of routingUrls) {
    console.error(`Testing routing bug: ${url}`);
  ...
  for (const item of paramUrls) {
    console.error(`Testing parameter URL: ${item.url}`);
  ...
  for (const path of revenuePages) {
    ...
    let metaRobots = "index,follow (default)";
    if (page && !error) {
      const robotsContent = await page.$eval('meta[name="robots"]', el => el.getAttribute('content')).catch(() => null);
      if (robotsContent) {
  ...
  fs.writeFileSync('/tmp/final_seo_results.json', JSON.stringify(results, null, 2));
