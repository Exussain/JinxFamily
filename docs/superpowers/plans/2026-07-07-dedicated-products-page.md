# صفحه اختصاصی محصولات نوبیکس شاپ - برنامه پیاده‌سازی

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** ایجاد یک صفحه محصولات اختصاصی در آدرس `/products` با همگونی کامل ظاهری با صفحه اصلی و صفحات دسته‌بندی، و تغییر لینک‌های هدایت هدبار دسکتاپ و موبایل از `/vbucks` به این صفحه جدید.

**Architecture:** صفحه جدید به عنوان یک کامپوننت سروری Next.js (`page.jsx`) ساخته می‌شود. در سمت سرور (SSR) با متد موازی `Promise.all` داده‌های دسته‌بندی‌ها و محصولات مربوطه را لود می‌کند و با رندر یک هیرو بنر شیشه‌ای شکیل و کارت‌های دسته‌بندی گرادینت به همراه لیست محصولات پرفروش، تجربه کاربری روان و بهینه‌ای برای سئو ایجاد می‌نماید.

**Tech Stack:** Next.js (App Router), React, Vanilla CSS.

## Global Constraints

- پس از هر تغییر در بخش فرانت‌اند، باید فایل `/root/NubixShop/public/HardReload.sh` اجرا شود تا برنامه کامپایل شده و سرور PM2 ری‌استارت شود.
- هر تغییر فرانت‌اند باید به همراه یک لاگ به زبان فارسی و با تاریخ روز در فایل `frontend/CHANGELOG.md` ثبت شود.
- تمام سبک‌ها و استایل‌های جدید درون خود فایل صفحه به صورت تگ `<style>` ایزوله یا درون‌خطی قرار می‌گیرند تا فایل عظیم `globals.css` دستخوش تغییرات ناخواسته نگردد.

---

### Task 1: پیاده‌سازی صفحه جدید محصولات

**Files:**
- Create: `frontend/app/products/page.jsx`

**Interfaces:**
- Consumes: `/api/categories` و `/api/categories/${code}` از بک‌اند از طریق تابع `fetchApiJson` در `frontend/lib/serverFetch.mjs`
- Produces: صفحه با آدرس فیزیکی `/products` در مرورگر

- [ ] **Step 1: ایجاد فایل کامپوننت صفحه محصولات**
  محتوای کامل فایل `frontend/app/products/page.jsx` را به صورت کامپوننت سروری ایجاد کنید. کد باید از کانتینر اصلی، تگ‌های معنایی، و کلاس‌های `.category-hero` جهت یکپارچگی کامل با سایت استفاده کند.
...
- [ ] **Step 2: تایید ایجاد فایل**
  بررسی کنید که فایل در مسیر درست ساخته شده باشد و ساختار آن فاقد خطاهای نحوی جاوااسکریپت باشد.

- [ ] **Step 3: کامپایل لوکال**
  دستور `npm run build` را در پوشه `frontend` تست کنید تا مطمئن شوید کامپوننت جدید بدون خطا بیلد می‌شود.
  اجرا در ترمینال: `cd /root/NubixShop/public/frontend && NEXT_DIST_DIR=.next-build npm run build`
  خروجی مورد انتظار: تکمیل بیلد بدون خطای Lint یا Compile.

---

### Task 2: تغییر لینک‌ها در کامپوننت ناوبری (Navbar)

**Files:**
- Modify: `frontend/components/Navbar.jsx`

**Interfaces:**
- Consumes: دکمه محصولات در هدر دسکتاپ و موبایل
- Produces: هدایت کاربر به `/products` به جای `/vbucks`

- [ ] **Step 1: اعمال ویرایش در Navbar.jsx**
  در فایل `frontend/components/Navbar.jsx` تغییرات زیر را انجام دهید:
  - خط ۹۲۷: آدرس لینک `Link href="/vbucks"` مربوط به «محصولات» را به `/products` تغییر دهید.
  - خط ۱۰۳۰: آدرس لینک موبایل `Link href="/vbucks"` مربوط به «پرفروش‌ها» را به همان صورت حفظ کنید، یا اگر نیاز است نام و لینک آن را به محصولات اختصاصی تغییر دهید (کاربر در هدبار دسکتاپ را خواسته است، پس دسکتاپ در خط ۹۲۷ الزامی است).

- [ ] **Step 2: صحت‌سنجی کدهای اصلاح‌شده**
  تأیید کنید تگ‌های `Link` همچنان شامل ویژگی‌های `onClick={() => setShowMobileMenu(false)}` باشند تا منو پس از کلیک بسته شود.

---

### Task 3: ثبت لاگ تغییرات و استقرار روی سرور

**Files:**
- Modify: `frontend/CHANGELOG.md`
- Run: `/root/NubixShop/public/HardReload.sh`

- [ ] **Step 1: ثبت تاریخچه در CHANGELOG.md**
  یک بخش جدید با عنوان تاریخ جاری (۲۰۲۶-۰۷-۰۷) و شرح اقدامات فارسی اضافه کنید:
  ```markdown
  ## 2026-07-07

  ### صفحه محصولات اختصاصی
  - **`frontend/components/Navbar.jsx`**: تغییر لینک «محصولات» در هدر از `/vbucks` به `/products`.
  - **`frontend/app/products/page.jsx`**: ایجاد صفحه دایرکتوری جدید محصولات با رویکرد الف (شامل دسته‌بندی‌ها و لیست محصولات برجسته هر دسته).
  ```

- [ ] **Step 2: اجرای اسکریپت استقرار نهایی**
  اسکریپت بارگذاری مجدد سرور را اجرا کنید:
  فرمان: `bash /root/NubixShop/public/HardReload.sh`
  خروجی مورد انتظار: بیلد موفقیت‌آمیز اپلیکیشن Next.js و نمایش پیغام ری‌استارت موفق pm2.

- [ ] **Step 3: تست نهایی زنده**
  با استفاده از `curl` آدرس محلی را تست کنید تا مطمئن شوید کد وضعیت ۲۰۰ برمی‌گردد:
  فرمان: `curl -I http://127.0.0.1:3002/products`
  خروجی مورد انتظار: `HTTP/1.1 200 OK`
